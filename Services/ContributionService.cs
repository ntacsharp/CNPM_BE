...
namespace CNPM_BE.Services
{
    public class ContributionService
    {
        ...
        {
            var resp = new ApiResponseExpose<ContributionResp>();
            var contribution = await _context.Contribution.FirstOrDefaultAsync(c => c.Id == req.Id && c.Status != ContributionStatus.Deleted);
        ...
        {
            var resp = new ApiResponseExpose<ContributionResp>();
            var contribution = await _context.Contribution.FirstOrDefaultAsync(c => c.Id == req && c.Status != ContributionStatus.Deleted);
            if(contribution == null)
            {
                resp.code = -1;
                resp.message = "Đã có lỗi xảy ra trong quá trình tìm kiếm đóng góp";
        ...
        {
            var list = await _context.Contribution.Where(c => c.Status != ContributionStatus.Deleted).ToListAsync();
