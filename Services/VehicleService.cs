...
namespace CNPM_BE.Services
{
    public class VehicleService
    {
        ...
        public async Task<ApiResponseExpose<VehicleResp>> AddVehicle(AppUser user, VehicleCreateReq req)
        {
            ...
            var owner = await _context.Resident.FirstOrDefaultAsync(r => r.Id == req.OwnerId && r.Status != ResidentStatus.Deleted);
            var ex = await _context.Vehicle.FirstOrDefaultAsync(v => v.Plate == req.Plate && v.Status != VehicleStatus.Deleted);
            if (ex != null)
            {
                resp.code = -1;
